...
export default function ItemDetail() {
  ...
  useEffect(() => {
    if (item?.title) {
      ...
      meta.content = desc;
    } else if (notFound) document.title = "404 Not Found | Bookify";
    else document.title = "Loading Item... | Bookify";
  }, [item, notFound]);
  ...
  const handleBuyNow = async () => {
    ...
    try {
      ...
      if (paymentRes.data.data?.redirect_url) window.location.href = paymentRes.data.data.redirect_url;
      else {
  ...
  const images = item.images?.length
    ? item.images
    : Array.from({ length: 12 }, (_, i) => `https://picsum.photos/seed/${item.id}-${i + 1}/400/550`);
